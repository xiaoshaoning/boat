...
#define TRAIN_OFFSET_LAYER(m, l)       (TRAIN_OFFSET_LMHEAD(m) + (size_t)(m)->vocab_size * (m)->hidden_size \
...
#define TRAIN_OFFSET_FC2(m, l)         (TRAIN_OFFSET_FC1(m, l) + (size_t)(m)->intermediate_size * (m)->hidden_size)
#define TRAIN_GRAD_TOTAL(m)            (TRAIN_OFFSET_FC2(m, (m)->n_layers - 1) \
                                        + (size_t)(m)->intermediate_size * (m)->hidden_size)
...
// -----------------------------------------------------------------------
#define ACT_OFFSET_HIN(m, l, T)        ((size_t)(l) * (6ULL * (size_t)(T) * (m)->hidden_size + (size_t)(T) * (m)->intermediate_size))
...
#define ACT_LAYER_STRIDE(m, T)         (6ULL * (size_t)(T) * (m)->hidden_size + (size_t)(T) * (m)->intermediate_size)
