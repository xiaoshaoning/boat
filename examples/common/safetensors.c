...
int safetensors_open(safetensors_t* st, const char* filename) {
    ...
    if (fs < 0) {
        fprintf(stderr, "[ERROR] ftell64 failed\n");
        fclose(f);
        return 0;
    }
    ...
    for (int i = 0; i < 8; i++)
        header_len |= ((uint64_t)st->file_data[i]) << (i * 8);
    ...
    while (1) {
        ...
        for (int k = 0; k < 10; k++) {
            ...
            if (strcmp(kname, "dtype") == 0) {
                ...
                if (dtype_str) {
                    strncpy(info.dtype, dtype_str, sizeof(info.dtype) - 1);
                    free(dtype_str);
                }
                ...
                if (json_next(&jctx) == '[') {
                    ...
                    while (1) {
                        ...
                        if (info.ndim >= shape_cap) {
                            ...
                            int64_t* tmp =
                                (int64_t*)realloc(info.shape, sizeof(int64_t) * shape_cap);
                            if (!tmp) {
                                free(kname);
                                break;
                            }
        ...
        if (st->count >= capacity) {
            capacity *= 2;
            st->tensors =
                (st_tensor_info_t*)realloc(st->tensors, sizeof(st_tensor_info_t) * capacity);
        }
...
boat_tensor_t* safetensors_load_tensor(const safetensors_t* st, int idx, int do_transpose) {
    ...
    boat_tensor_t* t =
        boat_tensor_create(info->shape, (size_t)info->ndim, tensor_dtype, BOAT_DEVICE_CPU);
    if (!t) return NULL;
    if (need_i64) {
