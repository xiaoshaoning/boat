...
static void build_sliding_windows(const float* seq, size_t seq_len, size_t window_size,
...
static void register_reg_params(boat_optimizer_t* opt, reg_model_t* m) {
    boat_optimizer_add_parameter(opt, boat_dense_layer_get_weight(m->fc1),
                                 boat_dense_layer_get_grad_weight(m->fc1));
    boat_optimizer_add_parameter(opt, boat_dense_layer_get_bias(m->fc1),
                                 boat_dense_layer_get_grad_bias(m->fc1));
    boat_optimizer_add_parameter(opt, boat_dense_layer_get_weight(m->fc2),
                                 boat_dense_layer_get_grad_weight(m->fc2));
    boat_optimizer_add_parameter(opt, boat_dense_layer_get_bias(m->fc2),
                                 boat_dense_layer_get_grad_bias(m->fc2));
    boat_optimizer_add_parameter(opt, boat_dense_layer_get_weight(m->fc3),
                                 boat_dense_layer_get_grad_weight(m->fc3));
    boat_optimizer_add_parameter(opt, boat_dense_layer_get_bias(m->fc3),
                                 boat_dense_layer_get_grad_bias(m->fc3));
}
...
static int run_regression_demo(void) {
    ...
    boat_tensor_t* train_x =
        boat_tensor_create(train_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
    boat_tensor_t* train_y =
        boat_tensor_create(train_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
    ...
    boat_tensor_t* batch_x =
        boat_tensor_create(batch_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
    boat_tensor_t* batch_y =
        boat_tensor_create(batch_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
    if (!batch_x || !batch_y) {
        fprintf(stderr, "Failed batch tensors\n");
        return 1;
    }
...
static int run_timeseries_demo(void) {
    ...
    boat_tensor_t* train_x =
        boat_tensor_create(train_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
    boat_tensor_t* test_x = boat_tensor_create(test_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
    boat_tensor_t* train_y_t =
        boat_tensor_create(train_y_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
    boat_tensor_t* test_y_t =
        boat_tensor_create(test_y_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
    ...
    memcpy(boat_tensor_data(test_x), X + n_train * window_size,
           n_test * window_size * sizeof(float));
    ...
    boat_tensor_t* batch_x =
        boat_tensor_create(batch_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
    boat_tensor_t* batch_yt =
        boat_tensor_create(batch_y_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
    if (!batch_x || !batch_yt) {
        fprintf(stderr, "Failed batch\n");
        return 1;
    }
    ...
    for (int ep = 0; ep < epochs; ep++) {
        for (size_t b = 0; b < n_batches; b++) {
            ...
            memcpy(boat_tensor_data(batch_yt), train_y_data + off, batch_size * sizeof(float));
    ...
    printf("\nMulti-step iterative prediction (error accumulates):\n");
    {
        ...
        for (size_t step = 0; step < 20; step++) {
            ...
            boat_tensor_t* input_t =
                boat_tensor_create(single_shape, 2, BOAT_DTYPE_FLOAT32, BOAT_DEVICE_CPU);
            if (!input_t) break;
