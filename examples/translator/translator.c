...
// ============================================================
static boat_tensor_t* layer_norm(const boat_tensor_t* x, const boat_tensor_t* w,
                                 const boat_tensor_t* b, float eps);
static boat_tensor_t* linear(const boat_tensor_t* x, const boat_tensor_t* w,
                             const boat_tensor_t* b);
static boat_tensor_t* embedding_lookup(const boat_tensor_t* weight, const int* ids, int batch,
                                       int seq_len);
...
// ============================================================
static int load_weight(marian_t* m, safetensors_t* st, const char* name, boat_tensor_t** w,
                       int do_transpose) {
    (void)m;
...
static int load_bias(marian_t* m, safetensors_t* st, const char* name, boat_tensor_t** b) {
    return load_weight(m, st, name, b, 0);
}

static int load_linear(marian_t* m, safetensors_t* st, const char* w_name, const char* b_name,
                       boat_tensor_t** w, boat_tensor_t** b) {
    return load_weight(m, st, w_name, w, 1) && load_bias(m, st, b_name, b);
}

static int load_attn(marian_t* m, safetensors_t* st, const char* prefix, int layer,
                     const char* attn_type, boat_tensor_t** qw, boat_tensor_t** qb,
                     boat_tensor_t** kw, boat_tensor_t** kb, boat_tensor_t** vw, boat_tensor_t** vb,
...
static int load_model(marian_t* m, safetensors_t* st) {
    ...
    for (int i = 0; i < N_LAYERS; i++) {
        // Self-attention
        if (!load_attn(m, st, "model.encoder.layers", i, "self_attn", &m->enc_q_w[i],
                       &m->enc_q_b[i], &m->enc_k_w[i], &m->enc_k_b[i], &m->enc_v_w[i],
                       &m->enc_v_b[i], &m->enc_o_w[i], &m->enc_o_b[i]))
            return 0;
    ...
    for (int i = 0; i < N_LAYERS; i++) {
        // Self-attention
        if (!load_attn(m, st, "model.decoder.layers", i, "self_attn", &m->dec_self_q_w[i],
                       &m->dec_self_q_b[i], &m->dec_self_k_w[i], &m->dec_self_k_b[i],
                       &m->dec_self_v_w[i], &m->dec_self_v_b[i], &m->dec_self_o_w[i],
                       &m->dec_self_o_b[i]))
            return 0;
        ...
        // Cross-attention
        if (!load_attn(m, st, "model.decoder.layers", i, "encoder_attn", &m->dec_cross_q_w[i],
                       &m->dec_cross_q_b[i], &m->dec_cross_k_w[i], &m->dec_cross_k_b[i],
                       &m->dec_cross_v_w[i], &m->dec_cross_v_b[i], &m->dec_cross_o_w[i],
                       &m->dec_cross_o_b[i]))
            return 0;
...
#define TF(x)                                                                                      \
    ...
    for (int i = 0; i < N_LAYERS; i++) {
        ...
        TF(m->enc_o_w[i]);
        TF(m->enc_o_b[i]);
        TF(m->enc_attn_ln_w[i]);
        TF(m->enc_attn_ln_b[i]);
        TF(m->enc_fc1_w[i]);
        TF(m->enc_fc1_b[i]);
        TF(m->enc_fc2_w[i]);
        TF(m->enc_fc2_b[i]);
        ...
        TF(m->dec_self_o_w[i]);
        TF(m->dec_self_o_b[i]);
        TF(m->dec_self_ln_w[i]);
        TF(m->dec_self_ln_b[i]);

        TF(m->dec_cross_q_w[i]);
        TF(m->dec_cross_q_b[i]);
        TF(m->dec_cross_k_w[i]);
        TF(m->dec_cross_k_b[i]);
        TF(m->dec_cross_v_w[i]);
        TF(m->dec_cross_v_b[i]);
        TF(m->dec_cross_o_w[i]);
        TF(m->dec_cross_o_b[i]);
        TF(m->dec_cross_ln_w[i]);
        TF(m->dec_cross_ln_b[i]);
...
// ============================================================
static boat_tensor_t* encoder_forward(marian_t* m, const int* input_ids, int batch, int seq_len) {
    ...
    for (int layer = 0; layer < N_LAYERS; layer++) {
        ...
        boat_tensor_t* attn_out = NULL;
        {
            ...
            if (!Q_reshaped || !K_reshaped || !V_reshaped) {
                boat_tensor_unref(Q_reshaped);
                boat_tensor_unref(K_reshaped);
                boat_tensor_unref(V_reshaped);
                boat_tensor_unref(x);
                return NULL;
            }
            ...
            boat_tensor_t* attn =
                boat_scaled_dot_product_attention(Q_mh, K_mh, V_mh, scale, NULL, 0, 0.0f);
            boat_tensor_unref(Q_mh);
            boat_tensor_unref(K_mh);
            boat_tensor_unref(V_mh);
            if (!attn) {
...
static boat_tensor_t* decoder_step(marian_t* m, const boat_tensor_t* decoder_state,
                                   const boat_tensor_t* encoder_out, int batch, int seq_len) {
    ...
    for (int layer = 0; layer < N_LAYERS; layer++) {
        ...
        boat_tensor_unref(V);
        if (!Q_r || !K_r || !V_r) {
            boat_tensor_unref(Q_r);
            boat_tensor_unref(K_r);
            boat_tensor_unref(V_r);
            boat_tensor_unref(x);
            return NULL;
        }
        ...
        float scale = 1.0f / sqrtf((float)HEAD_DIM);
        boat_tensor_t* attn =
            boat_scaled_dot_product_attention(Q_mh, K_mh, V_mh, scale, NULL, 1, 0.0f);
        boat_tensor_unref(Q_mh);
        boat_tensor_unref(K_mh);
        boat_tensor_unref(V_mh);
        if (!attn) {
        ...
        boat_tensor_t* ln2 =
            layer_norm(x, m->dec_cross_ln_w[layer], m->dec_cross_ln_b[layer], 1e-5f);
        boat_tensor_unref(x);
        if (!ln2) return NULL;
        x = ln2;
...
// ============================================================
static int* greedy_decode(marian_t* m, const boat_tensor_t* encoder_hidden, int* out_len) {
    int batch = 1;
...
// ============================================================
static int translate_and_print(marian_t* model, spm_tokenizer_t* tok, const char* text) {
    int in_len;
...
int main(int argc, char** argv) {
    ...
    if (argc > text_arg && argv[text_arg][0] != '-') {
        ...
        if (verbose)
            fprintf(stderr,
                    "[INFO] Interactive mode. Type text to translate, or 'quit' to exit.\n");
        char line[4096];
        while (1) {
            fprintf(stderr, "> ");
