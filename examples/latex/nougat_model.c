...
#define LOAD(dst, name, transpose) do { \
    dst = load_weight(st, name, transpose); \
    if (!dst) { nougat_model_free(model); safetensors_close(st); return NULL; } \
} while(0)

// Load an optional weight (may be NULL).
#define LOAD_OPT(dst, name, transpose) do { \
    dst = load_weight(st, name, transpose); \
} while(0)
...
static int load_swin_block(safetensors_t* st, nougat_model_t* model,
                           boat_swin_block_weights_t* block,
                           const char* prefix, int dim, int num_heads, int ws)
{
    char key[512];
...
static int load_decoder_layer(safetensors_t* st, nougat_model_t* model,
                              boat_decoder_layer_weights_t* layer,
                              const char* prefix)
{
    char key[512];
...
nougat_model_t* nougat_model_create(const char* model_dir) {
    ...
    if (!model) { safetensors_close(st); return NULL; }
    ...
    if (!model->encoder) { nougat_model_free(model); safetensors_close(st); return NULL; }
    ...
    for (int s = 0; s < 4; s++) {
        ...
        model->encoder->stages[s].blocks = (boat_swin_block_weights_t*)calloc(
            num_blocks, sizeof(boat_swin_block_weights_t));

        for (int b = 0; b < num_blocks; b++) {
            char prefix[256];
            snprintf(prefix, sizeof(prefix), "encoder.encoder.layers.%d.blocks.%d", s, b);
            if (!load_swin_block(st, model, &model->encoder->stages[s].blocks[b], prefix, dim, heads,
                                 model->swin_config.window_size)) {
                nougat_model_free(model); safetensors_close(st); return NULL;
            }
        }

        // Downsample (stages 0-2 only)
        if (s < 3) {
            model->encoder->stages[s].downsample = (boat_swin_downsample_weights_t*)calloc(
                1, sizeof(boat_swin_downsample_weights_t));
    ...
    for (int l = 0; l < NUM_DECODER_LAYERS; l++) {
        model->decoder_layers[l] = (boat_decoder_layer_weights_t*)calloc(
            1, sizeof(boat_decoder_layer_weights_t));
        char prefix[256];
...
#define TO_DEVICE(tptr) do { \
    if (*(tptr)) { \
        t = boat_tensor_to_device(*(tptr), device); \
        if (!t) { fprintf(stderr, "[Nougat] Failed to transfer tensor to device\n"); return 0; } \
    ...
    for (int l = 0; l < model->num_decoder_layers; l++) {
        ...
        TO_DEVICE(&layer->self_q_weight); TO_DEVICE(&layer->self_q_bias);
        TO_DEVICE(&layer->self_k_weight); TO_DEVICE(&layer->self_k_bias);
        TO_DEVICE(&layer->self_v_weight); TO_DEVICE(&layer->self_v_bias);
        TO_DEVICE(&layer->self_o_weight); TO_DEVICE(&layer->self_o_bias);
        TO_DEVICE(&layer->self_ln_weight); TO_DEVICE(&layer->self_ln_bias);
        TO_DEVICE(&layer->cross_q_weight); TO_DEVICE(&layer->cross_q_bias);
        TO_DEVICE(&layer->cross_k_weight); TO_DEVICE(&layer->cross_k_bias);
        TO_DEVICE(&layer->cross_v_weight); TO_DEVICE(&layer->cross_v_bias);
        TO_DEVICE(&layer->cross_o_weight); TO_DEVICE(&layer->cross_o_bias);
        TO_DEVICE(&layer->cross_ln_weight); TO_DEVICE(&layer->cross_ln_bias);
        TO_DEVICE(&layer->fc1_weight); TO_DEVICE(&layer->fc1_bias);
        TO_DEVICE(&layer->fc2_weight); TO_DEVICE(&layer->fc2_bias);
        TO_DEVICE(&layer->ffn_ln_weight); TO_DEVICE(&layer->ffn_ln_bias);
    }
