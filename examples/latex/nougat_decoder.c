...
static boat_tensor_t* embed_token(const boat_tensor_t* weight, int32_t token_id,
    ...
    if (device == BOAT_DEVICE_CPU) {
        ...
        float* src_ptr =
            (float*)boat_tensor_const_data(weight) + (size_t)token_id * (size_t)d_model;
...
// ---------------------------------------------------------------------------
static boat_tensor_t* embed_position(const boat_tensor_t* weight, int position,
                                     boat_device_t device) {
    const float* w = (const float*)boat_tensor_const_data(weight);
    int64_t d_model = boat_tensor_shape(weight)[1];
    int64_t shape[] = {1, 1, d_model};
    ...
    if (device == BOAT_DEVICE_CPU) {
        ...
        float* src_ptr =
            (float*)boat_tensor_const_data(weight) + (size_t)position * (size_t)d_model;
        boat_cuda_copy_to_device(data, src_ptr, (size_t)d_model * sizeof(float));
    }
#endif
    return out;
...
// ---------------------------------------------------------------------------
static boat_tensor_t* ln_affine(const boat_tensor_t* x, const boat_tensor_t* gamma,
                                const boat_tensor_t* beta, float eps, boat_device_t device) {
    (void)device;
...
// Returns: [..., out]
// ---------------------------------------------------------------------------
static boat_tensor_t* linear_fwd(const boat_tensor_t* x, const boat_tensor_t* weight,
...
// ---------------------------------------------------------------------------
static boat_tensor_t* decoder_stack_forward(const nougat_model_t* model, const boat_tensor_t* input,
                                            const boat_tensor_t* encoder_output,
                                            boat_decoder_cache_t** caches, int step,
                                            boat_device_t device) {
    ...
    for (int l = 0; l < model->num_decoder_layers; l++) {
        boat_tensor_t* next = boat_decoder_layer_forward(cfg, model->decoder_layers[l], x,
                                                         encoder_output, caches[l], step);
        boat_tensor_unref(x);
        if (!next) return NULL;
        x = next;
    }
...
int nougat_decoder_generate(const nougat_model_t* model, const boat_tensor_t* encoder_output,
    ...
    for (int l = 0; l < model->num_decoder_layers; l++) {
        caches[l] =
            boat_decoder_cache_create_ex(batch_size, H, head_dim, max_T, enc_seq_len, device);
        if (!caches[l]) {
            for (int k = 0; k < l; k++)
                boat_decoder_cache_free(caches[k]);
            free(caches);
            free(ids);
            return -1;
        }
    ...
    for (int step = 0; step < max_steps; step++) {
        ...
        // Decoder stack
        boat_tensor_t* dec_out =
            decoder_stack_forward(model, h, encoder_output, caches, step, device);
        boat_tensor_unref(h);
        if (!dec_out) {
            free(ids);
            ids = NULL;
