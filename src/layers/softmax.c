...
BOAT_API BOAT_API boat_tensor_t* BOAT_CALL boat_softmax_layer_backward(boat_softmax_layer_t* layer, const boat_tensor_t* grad_output) {
    ...
    size_t inner = 1;
    for (int i = axis + 1; i < (int)ndim; i++) inner *= (size_t)shape[i];
    ...
    switch (dtype) {
        case BOAT_DTYPE_FLOAT32: {
            ...
            for (size_t o = 0; o < outer; o++) {
                for (size_t i = 0; i < inner; i++) {
                    ...
                    for (size_t k = 0; k < axis_size; k++) {
                        out_ptr[base + k * inner] = y_ptr[base + k * inner] * (g_ptr[base + k * inner] - sum_y_g);
                    }
                }
            }
            break;
        ...
        default:
            ...
            boat_set_errorf(BOAT_ERROR_UNKNOWN, "[SoftmaxLayer] Backward only supports float32/float64\n");
            return NULL;
    }
