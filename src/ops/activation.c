...
BOAT_API boat_tensor_t* boat_softmax(const boat_tensor_t* a, int axis) {
    ...
    if (boat_tensor_device(a) == BOAT_DEVICE_CUDA && dtype == BOAT_DTYPE_FLOAT32) {
        boat_cuda_softmax_f32((const float*)boat_tensor_const_data(a),
                              (float*)boat_tensor_data(out), (int64_t)outer_elements,
                              (int64_t)axis_size, (int64_t)inner_stride);
        return out;
    }
#endif
...
#ifdef BOAT_WITH_CUDA
    if (boat_tensor_device(a) == BOAT_DEVICE_CUDA && dtype == BOAT_DTYPE_FLOAT32) {
        boat_cuda_log_softmax_f32((const float*)a_data, (float*)out_data, (int64_t)outer_elements,
                                  (int64_t)axis_size, (int64_t)inner_stride);
        return out;
    }
#endif
...
#ifdef BOAT_WITH_CUDA
    if (boat_tensor_device(a) == BOAT_DEVICE_CUDA && dtype == BOAT_DTYPE_FLOAT32) {
        boat_cuda_silu_f32((const float*)boat_tensor_const_data(a), (float*)boat_tensor_data(out),
                           n);
        return out;
    }
#endif
...
#ifdef BOAT_WITH_CUDA
    if (boat_tensor_device(a) == BOAT_DEVICE_CUDA && dtype == BOAT_DTYPE_FLOAT32) {
        boat_cuda_tanh_f32((const float*)boat_tensor_const_data(a), (float*)boat_tensor_data(out),
                           n);
        return out;
    }
#endif
...
#ifdef BOAT_WITH_CUDA
    if (boat_tensor_device(a) == BOAT_DEVICE_CUDA && dtype == BOAT_DTYPE_FLOAT32) {
        boat_cuda_gelu_f32((const float*)boat_tensor_const_data(a), (float*)boat_tensor_data(out),
                           n);
        return out;
    }
#endif
...
BOAT_API boat_tensor_t* boat_sinusoidal_embedding(size_t seq_len, size_t embedding_dim,
                                                  float theta) {
