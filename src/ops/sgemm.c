...
BOAT_API void boat_sgemm(int64_t M, int64_t N, int64_t K, const float* A, const float* B,
                         float* C) {
    if (M <= 0 || N <= 0 || K <= 0) return;
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, (int)M, (int)N, (int)K, 1.0f, A, (int)K,
                B, (int)N, 0.0f, C, (int)N);
}

BOAT_API void boat_sgemm_batched(int64_t batch, int64_t M, int64_t N, int64_t K, const float* A,
                                 int64_t stride_a, const float* B, int64_t stride_b, float* C,
                                 int64_t stride_c) {
    ...
    for (int64_t b = 0; b < batch; b++) {
        boat_sgemm(M, N, K, A + b * stride_a, B + b * stride_b, C + b * stride_c);
    }
}
...
static void pack_a_mr(int64_t mc, int64_t kc, const float* A, int64_t ldA, float* packed,
                      int64_t mr) {
...
static void pack_b_nr(int64_t nc, int64_t kc, const float* B, int64_t ldB, float* packed,
                      int64_t nr) {
...
static void micro_4x8(int64_t kc, const float* A_packed, const float* B_packed, float* C,
                      int64_t ldC) {
...
static void micro_4x8(int64_t kc, const float* A_packed, const float* B_packed, float* C,
    ...
    for (int64_t k = 0; k < kc; k++) {
        ...
        c0l = vmlaq_f32(c0l, a0, bl);
        c0h = vmlaq_f32(c0h, a0, bh);
        c1l = vmlaq_f32(c1l, a1, bl);
        c1h = vmlaq_f32(c1h, a1, bh);
        c2l = vmlaq_f32(c2l, a2, bl);
        c2h = vmlaq_f32(c2h, a2, bh);
        c3l = vmlaq_f32(c3l, a3, bl);
        c3h = vmlaq_f32(c3h, a3, bh);
    }
...
static void micro_remainder_rows(int64_t mr, int64_t nc, int64_t kc, const float* A_packed,
                                 const float* B_packed, float* C, int64_t ldC) {
...
static void micro_remainder_cols(int64_t mr, int64_t nc, int64_t kc, const float* A_packed,
                                 const float* B_packed, float* C, int64_t ldC) {
    for (int64_t r = 0; r < mr; r++) {
        for (int64_t c = 0; c < nc; c++) {
            float acc = C[r * ldC + c];
...
static void sgemm_panel(int64_t mc, int64_t nc, int64_t kc, const float* A_packed,
                        const float* B_packed, float* C, int64_t ldC) {
    ...
    for (int64_t i = 0; i < mc; i += mr) {
        ...
        for (int64_t j = 0; j < nc; j += nr) {
            ...
            if (rows >= mr && cols >= nr) {
                ...
                micro_remainder_cols(mr, cols, kc, A_panel, B_panel, C + i * ldC + j, ldC);
            } else {
                micro_remainder_rows(rows, cols < nr ? cols : nr, kc, A_panel, B_panel,
                                     C + i * ldC + j, ldC);
            }
        }
    }
    ...
    for (int64_t i = 0; i < M; i += mc) {
        ...
        for (int64_t j = 0; j < N; j += nc) {
            ...
            for (int64_t p = 0; p < K; p += kc) {
                ...
                sgemm_panel(mc_eff, nc_eff, kc_eff, a_packed, b_packed, C + i * N + j, N);
            }
        }
    }
