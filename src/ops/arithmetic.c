...
static bool validate_shapes_for_broadcasting(const boat_tensor_t* a, const boat_tensor_t* b,
...
static boat_tensor_t* create_broadcasted_output(const boat_tensor_t* a, const boat_tensor_t* b,
                                                boat_dtype_t dtype) {
    ...
    if (!validate_shapes_for_broadcasting(a, b, out_shape, &out_ndim)) {
        BOAT_DEBUG_PRINT(
            "DEBUG create_broadcasted_output: validate_shapes_for_broadcasting failed\n");
        // Debug shape info
        size_t a_ndim = boat_tensor_ndim(a);
        size_t b_ndim = boat_tensor_ndim(b);
        const int64_t* a_shape = boat_tensor_shape(a);
        const int64_t* b_shape = boat_tensor_shape(b);
        (void)a_shape;
        (void)b_shape; // used only in BOAT_DEBUG_PRINT below
    ...
    if (device != b_device) {
        boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,
                        "[Arithmetic] Device mismatch in broadcast: a=%d, b=%d\n", device,
                        b_device);
    ...
    do {                                                                                           \
        if (boat_tensor_device(a) == BOAT_DEVICE_CUDA) {                                           \
            if (dtype != BOAT_DTYPE_FLOAT32) {                                                     \
                boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,                                       \
                                "[Arithmetic] CUDA only supports float32 in boat_%s\n", #op_name); \
...
#define ELEMWISE_SCALAR_CUDA_FAST_PATH(cuda_func_call)                                             \
    do {                                                                                           \
        if (boat_tensor_device(a) == BOAT_DEVICE_CUDA) {                                           \
            if (dtype != BOAT_DTYPE_FLOAT32) {                                                     \
                boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,                                       \
                                "[Arithmetic] CUDA only supports float32 in scalar op\n");         \
                boat_tensor_free(out);                                                             \
                return NULL;                                                                       \
            }                                                                                      \
            cuda_func_call;                                                                        \
...
BOAT_API boat_tensor_t* boat_add(const boat_tensor_t* a, const boat_tensor_t* b) {
...
#ifdef BOAT_WITH_CUDA
    ELEMWISE_CUDA_FAST_PATH(add, boat_cuda_add_f32((const float*)a_data, (const float*)b_data,
                                                   (float*)out_data, nelements));
#endif
    ...
    default:
        boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,
                        "[Arithmetic] Unsupported dtype in boat_add: %d\n", dtype);
        boat_tensor_free(out);
        return NULL;
    }
    return out;
...
BOAT_API boat_tensor_t* boat_sub(const boat_tensor_t* a, const boat_tensor_t* b) {
    ...
    void* out_data = boat_tensor_data(out);
#ifdef BOAT_WITH_CUDA
    ELEMWISE_CUDA_FAST_PATH(sub, boat_cuda_sub_f32((const float*)a_data, (const float*)b_data,
                                                   (float*)out_data, nelements));
#endif
    const int64_t* out_shape = boat_tensor_shape(out);
    size_t out_ndim = boat_tensor_ndim(out);
    ...
    case BOAT_DTYPE_FLOAT16: {
        ...
        for (size_t i = 0; i < nelements; i++) {
            ...
            size_t b_idx = broadcast_index(b, i, out_shape, out_ndim);
            out_ptr[i] =
                boat_f32_to_f16(boat_f16_to_f32(a_ptr[a_idx]) - boat_f16_to_f32(b_ptr[b_idx]));
        }
        break;
    }
    case BOAT_DTYPE_BFLOAT16: {
        ...
        for (size_t i = 0; i < nelements; i++) {
            ...
            size_t b_idx = broadcast_index(b, i, out_shape, out_ndim);
            out_ptr[i] =
                boat_f32_to_bf16(boat_bf16_to_f32(a_ptr[a_idx]) - boat_bf16_to_f32(b_ptr[b_idx]));
        }
        break;
    }
    default:
        boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,
                        "[Arithmetic] Unsupported dtype in boat_sub\n");
        boat_tensor_free(out);
        return NULL;
    }
    return out;
...
BOAT_API boat_tensor_t* boat_mul(const boat_tensor_t* a, const boat_tensor_t* b) {
    ...
    void* out_data = boat_tensor_data(out);
#ifdef BOAT_WITH_CUDA
    ELEMWISE_CUDA_FAST_PATH(mul, boat_cuda_mul_f32((const float*)a_data, (const float*)b_data,
                                                   (float*)out_data, nelements));
#endif
    const int64_t* out_shape = boat_tensor_shape(out);
    size_t out_ndim = boat_tensor_ndim(out);
    ...
    default:
        boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,
                        "[Arithmetic] Unsupported dtype in boat_mul\n");
        boat_tensor_free(out);
        return NULL;
    }
    return out;
...
BOAT_API boat_tensor_t* boat_div(const boat_tensor_t* a, const boat_tensor_t* b) {
    ...
    void* out_data = boat_tensor_data(out);
#ifdef BOAT_WITH_CUDA
    ELEMWISE_CUDA_FAST_PATH(div, boat_cuda_div_f32((const float*)a_data, (const float*)b_data,
                                                   (float*)out_data, nelements));
#endif
    const int64_t* out_shape = boat_tensor_shape(out);
    size_t out_ndim = boat_tensor_ndim(out);
    ...
    default:
        boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,
                        "[Arithmetic] Unsupported dtype in boat_div\n");
        boat_tensor_free(out);
        return NULL;
    }
    return out;
...
BOAT_API boat_tensor_t* boat_mod(const boat_tensor_t* a, const boat_tensor_t* b) {
    ...
    if (dtype != boat_tensor_dtype(b)) {
        boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,
                        "[Arithmetic] Dtype mismatch in boat_mod: %d vs %d\n", dtype,
                        boat_tensor_dtype(b));
        return NULL;
    }
    ...
    if (boat_tensor_device(a) == BOAT_DEVICE_CUDA && dtype == BOAT_DTYPE_FLOAT32) {
        if (boat_tensor_ndim(a) == boat_tensor_ndim(b) && boat_tensor_nelements(a) == nelements &&
            boat_tensor_nelements(b) == nelements) {
            boat_cuda_mod_f32((const float*)a_data, (const float*)b_data, (float*)out_data,
                              nelements);
            return out;
        }
    }
    ...
    case BOAT_DTYPE_FLOAT16: {
        ...
        for (size_t i = 0; i < nelements; i++) {
            size_t a_idx = broadcast_index(a, i, out_shape, out_ndim);
            size_t b_idx = broadcast_index(b, i, out_shape, out_ndim);
            out_ptr[i] = boat_f32_to_f16(
                fmodf(boat_f16_to_f32(a_ptr[a_idx]), boat_f16_to_f32(b_ptr[b_idx])));
        }
        break;
    }
    default:
        boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,
                        "[Arithmetic] Unsupported dtype in boat_mod\n");
        boat_tensor_free(out);
        return NULL;
    }
...
#define DEFINE_INPLACE_OP(op_name, op)                                                             \
    void boat_##op_name##_(boat_tensor_t* const a, const boat_tensor_t* b) {                       \
        ...
        if (dtype != boat_tensor_dtype(b)) {                                                       \
            boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,                                           \
                            "[Arithmetic] Dtype mismatch in boat_%s_: %d vs %d\n", #op_name,       \
...
#define DEFINE_SCALAR_OP(op_name, op)                                                              \
    boat_tensor_t* boat_##op_name##_scalar(const boat_tensor_t* a, double scalar) {                \
        if (!a) {                                                                                  \
            boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,                                           \
                            "[Arithmetic] Null input in boat_%s_scalar\n", #op_name);              \
            return NULL;                                                                           \
        }                                                                                          \
                                                                                                   \
        boat_dtype_t dtype = boat_tensor_dtype(a);                                                 \
        ...
        if (!out) {                                                                                \
            boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,                                           \
                            "[Arithmetic] Failed to create output in boat_%s_scalar\n", #op_name); \
            return NULL;                                                                           \
        }                                                                                          \
                                                                                                   \
        ...
        default:                                                                                   \
            boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,                                           \
                            "[Arithmetic] Unsupported dtype in boat_%s_scalar\n", #op_name);       \
            boat_tensor_free(out);                                                                 \
            return NULL;                                                                           \
        }                                                                                          \
                                                                                                   \
...
BOAT_API boat_tensor_t* boat_pow_scalar(const boat_tensor_t* a, double scalar) {
    if (!a) {
        boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,
                        "[Arithmetic] Null input in boat_pow_scalar\n");
        return NULL;
    }
...
BOAT_API boat_tensor_t* boat_broadcast_to(const boat_tensor_t* a, const int64_t* shape,
    ...
    if (!a || !shape) {
        boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT,
                        "[Arithmetic] Null input in boat_broadcast_to\n");
        return NULL;
    }
