...
BOAT_API boat_tensor_t* boat_matmul(const boat_tensor_t* a, const boat_tensor_t* b) {
    ...
    if (dtype != boat_tensor_dtype(b)) {
        boat_set_errorf(BOAT_ERROR_INVALID_ARGUMENT, "[Linear] matmul dtype mismatch: %d != %d\n",
    ...
    if (a_bd > 0) {
        a_bs[a_bd - 1] = 1;
        for (int i = (int)a_bd - 2; i >= 0; i--)
            a_bs[i] = a_bs[i + 1] * (size_t)a_shape[i + 1];
    }
    if (b_bd > 0) {
        b_bs[b_bd - 1] = 1;
        for (int i = (int)b_bd - 2; i >= 0; i--)
            b_bs[i] = b_bs[i + 1] * (size_t)b_shape[i + 1];
    }
    if (out_bd > 0) {
        out_bs[out_bd - 1] = 1;
        for (int i = (int)out_bd - 2; i >= 0; i--)
            out_bs[i] = out_bs[i + 1] * (size_t)out_shape[i + 1];
    }
...
#define MATMUL_BATCH_OFFSETS(ob, a_off, b_off)                                                     \
        ...
        if (boat_tensor_device(a) == BOAT_DEVICE_CUDA && a_bd == b_bd) {
            if (out_batch_nelems == 1) {
                ...
                boat_cuda_matmul_f32_strided_batched(
                    ...
                    (size_t)out_batch_nelems, (size_t)(m * k), (size_t)(k * n), (size_t)(m * n));
            }
        } else
#endif
        {
            for (size_t ob = 0; ob < out_batch_nelems; ob++) {
                ...
                boat_sgemm(m, n, k, a_ptr + a_off * (size_t)(m * k),
                           b_ptr + b_off * (size_t)(k * n), out_ptr + ob * (size_t)(m * n));
            }
        }
        break;
    }
    ...
    default: boat_tensor_unref(out); return NULL;
    }
...
BOAT_API boat_tensor_t* boat_transpose(const boat_tensor_t* a, int dim0, int dim1) {
    ...
    boat_tensor_t* out =
        boat_tensor_create(out_shape, ndim, boat_tensor_dtype(a), boat_tensor_device(a));
    boat_free(out_shape);
    ...
    if (boat_tensor_device(a) == BOAT_DEVICE_CUDA && boat_tensor_dtype(a) == BOAT_DTYPE_FLOAT32) {
        if (ndim == 2) {
            ...
            boat_cuda_transpose_f32((const float*)in_data, (float*)out_data, shape[0], shape[1]);
        } else {
            ...
            if (in_stride && out_stride && dev_shape) {
                ...
                boat_cuda_transpose_nd_f32((const float*)in_data, (float*)out_data,
                                           (int64_t)total_elements, dev_shape, in_stride,
                                           out_stride, (int64_t)ndim, dim0, dim1);
                free(in_stride);
                free(out_stride);
                free(dev_shape);
            }
