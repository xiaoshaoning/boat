...
static bool read_bool(const uint8_t** p) {
    return read_u8(p) != 0;
}
...
static bool dequantize_tensor(const uint8_t* src, float* dst, const gguf_tensor_info_t* info) {
    size_t n = info->n_elements;
    if (n == 0) return true;

    switch (info->type) {
    case GGML_TYPE_F32: memcpy(dst, src, n * 4); return true;
...
static gguf_context_t* gguf_parse(const uint8_t* data, size_t size) {
    ...
    if (metadata_kv_count > 0) {
        ctx->metadata =
            (gguf_kv_t*)boat_malloc((size_t)metadata_kv_count * sizeof(gguf_kv_t), BOAT_DEVICE_CPU);
        if (!ctx->metadata) {
            boat_free(ctx);
            return NULL;
        }
...
static const char* find_metadata_string(const gguf_context_t* ctx, const char* key) {
    for (uint64_t i = 0; i < ctx->metadata_kv_count; i++) {
        if (strcmp(ctx->metadata[i].key, key) == 0 && ctx->metadata[i].type == GGUF_TYPE_STRING) {
...
static boat_tensor_t* dequantize_and_transpose(const gguf_context_t* ctx,
                                               const gguf_tensor_info_t* info, int64_t in_features,
                                               int64_t out_features) {
    if (!info) return NULL;

    // Check if GGUF shape matches [out, in]
    bool needs_transpose = true;
    if (info->n_dimensions >= 2 && (int64_t)info->dimensions[0] == in_features &&
...
static boat_tensor_t* dequantize_1d(const gguf_context_t* ctx, const gguf_tensor_info_t* info,
                                    int64_t n) {
    if (!info) return NULL;
...
#define READ_CFG_INT(key, field, def)                                                              \
    do {                                                                                           \
        char buf[128];                                                                             \
        snprintf(buf, sizeof(buf), "%s.%s", cfg->arch, key);                                       \
        cfg->field = find_metadata_int(ctx, buf, def);                                             \
        if (cfg->field == (int64_t)def) cfg->field = find_metadata_int(ctx, key, def);             \
    } while (0)

#define READ_CFG_FLOAT(key, field, def)                                                            \
    do {                                                                                           \
        char buf[128];                                                                             \
        snprintf(buf, sizeof(buf), "%s.%s", cfg->arch, key);                                       \
        cfg->field = find_metadata_float(ctx, buf, def);                                           \
        if (cfg->field == (float)def - 1.0f) cfg->field = find_metadata_float(ctx, key, def);      \
    } while (0)
...
static boat_model_t* build_llama_model(const gguf_context_t* ctx, const gguf_model_config_t* cfg) {
    ...
    for (int64_t i = 0; i < n_layers; i++) {
        ...
        if (norm1_info) {
            boat_rmsnorm_config_t rcfg = {.normalized_shape = (size_t)hidden,
                                          .eps = cfg->norm_eps,
        ...
        if (q_info && o_info) {
            boat_attention_config_t acfg = {.hidden_size = (size_t)hidden,
            ...
            if (k_info) {
                ...
                if (wk) {
                    boat_attention_set_weight_k(attn, wk);
                    boat_tensor_unref(wk);
                }
            }
            ...
            if (v_info) {
                ...
                if (wv) {
                    boat_attention_set_weight_v(attn, wv);
                    boat_tensor_unref(wv);
                }
            }
            ...
            if (wo) {
                boat_attention_set_weight_o(attn, wo);
                boat_tensor_unref(wo);
            }
        ...
        if (gate_info) {
            boat_dense_layer_t* gate = boat_dense_layer_create((size_t)hidden, (size_t)d_ff, false);
            if (!gate) {
                boat_model_free(model);
                return NULL;
            }
            ...
            if (w) {
                boat_dense_layer_set_weight(gate, w);
                boat_tensor_unref(w);
            }
            boat_layer_t* layer = (boat_layer_t*)malloc(sizeof(boat_layer_t));
            layer->data = gate;
        ...
        if (up_info) {
            boat_dense_layer_t* up = boat_dense_layer_create((size_t)hidden, (size_t)d_ff, false);
            if (!up) {
                boat_model_free(model);
                return NULL;
            }
            ...
            if (w) {
                boat_dense_layer_set_weight(up, w);
                boat_tensor_unref(w);
            }
            boat_layer_t* layer = (boat_layer_t*)malloc(sizeof(boat_layer_t));
            layer->data = up;
        ...
        if (down_info) {
            boat_dense_layer_t* down = boat_dense_layer_create((size_t)d_ff, (size_t)hidden, false);
            if (!down) {
                boat_model_free(model);
                return NULL;
            }
            ...
            if (w) {
                boat_dense_layer_set_weight(down, w);
                boat_tensor_unref(w);
            }
            boat_layer_t* layer = (boat_layer_t*)malloc(sizeof(boat_layer_t));
            layer->data = down;
    ...
    if (out_norm) {
        boat_rmsnorm_config_t rcfg = {
            .normalized_shape = (size_t)hidden, .eps = cfg->norm_eps, .elementwise_affine = true};
        boat_rmsnorm_t* rms = boat_rmsnorm_create(&rcfg);
        if (rms) {
    ...
    if (out_proj && cfg->vocab_size > 0) {
        boat_dense_layer_t* out =
            boat_dense_layer_create((size_t)hidden, (size_t)cfg->vocab_size, false);
        if (out) {
            boat_tensor_t* w = dequantize_and_transpose(ctx, out_proj, hidden, cfg->vocab_size);
            if (w) {
                boat_dense_layer_set_weight(out, w);
                boat_tensor_unref(w);
            }
            boat_layer_t* layer = (boat_layer_t*)malloc(sizeof(boat_layer_t));
            layer->data = out;
    ...
    gguf_model_config_t* cfg_copy =
        (gguf_model_config_t*)boat_malloc(sizeof(gguf_model_config_t), BOAT_DEVICE_CPU);
    if (cfg_copy) {
