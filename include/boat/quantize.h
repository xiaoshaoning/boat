...
// For INT8 asymmetric:   scale = (max - min) / 255, zp = round(-min / scale), clamped to [-128,
// 127] For BITS2:            qmax=3, qmin=0, scale = (max - min) / 3, zp = round(-min / scale) For
// BITS1:            qmax=1, qmin=0, scale = (max - min) / 1, zp = round(-min / scale) For FLOAT4:
// scale=1, zp=0 (no affine quantization, returns immediately)
...
// Quantize a FP32 tensor to quant_dtype (UINT8, INT8, BITS2, BITS1, or FLOAT4) with per-tensor
// affine quantization. For BITS2: quantize to [0,3] then pack 4 values per byte via
// boat_pack_bits2. For BITS1: quantize to [0,1] then pack 8 values per byte via boat_pack_bits1.
...
BOAT_API bool boat_fake_quantize_per_channel(boat_tensor_t* tensor,
                                             const boat_quant_config_t* config, size_t channel_dim);
