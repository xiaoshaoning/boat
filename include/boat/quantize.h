...
typedef struct {
    boat_dtype_t quant_dtype;   // Target dtype (BOAT_DTYPE_UINT8)
    bool symmetric;             // true -> zero_point forced to 128 (UINT8)
} boat_quant_config_t;
...
// Quantize a FP32 tensor to UINT8 with per-tensor affine quantization.
// Returns a new tensor with dtype=UINT8, scale and zero_point set.
...
// The input tensor must have dtype=UINT8 and scale != 0.
...
// with quantized UINT8 equivalents. Bias tensors are left as FP32.
