...
static float compute_numerical_loss_gradient(boat_loss_t* loss, boat_tensor_t* pred,
                                             const boat_tensor_t* target, size_t idx,
                                             float epsilon) {
...
static int test_mse_forward(void) {
    ...
    if (fabsf(loss_val) > 1e-6f) {
        boat_tensor_unref(p1);
        boat_tensor_unref(t1);
        boat_tensor_unref(p3);
        boat_tensor_unref(t3);
        boat_loss_free(mse);
...
static int test_loss_edge_cases(void) {
    ...
    if (g != NULL) {
        printf("FAIL: backward(NULL) != NULL\n");
        boat_tensor_unref(pred);
        boat_tensor_unref(target);
        return 1;
    }
