// test_optimizer_rmsprop.c - RMSprop optimizer unit tests
...
int main() {
    ...
    // Test 1: RMSprop optimizer creation
    {
        printf("Calling boat_rmsprop_optimizer_create...\n");

        boat_optimizer_t* optimizer = boat_rmsprop_optimizer_create(0.001f, 0.99f, 1e-8f);
        printf("Optimizer pointer: %p\n", optimizer);

        // Debug: also print to stderr
        fprintf(stderr, "Optimizer pointer (stderr): %p\n", optimizer);

        assert(optimizer != NULL);

        // Test that invalid parameters are rejected
        boat_optimizer_t* invalid1 = boat_rmsprop_optimizer_create(0.0f, 0.99f, 1e-8f);
        assert(invalid1 == NULL);

        boat_optimizer_t* invalid2 = boat_rmsprop_optimizer_create(0.001f, 1.1f, 1e-8f);
        assert(invalid2 == NULL);

        boat_optimizer_t* invalid3 = boat_rmsprop_optimizer_create(0.001f, 0.0f, 1e-8f);
        assert(invalid3 == NULL);

        boat_optimizer_free(optimizer);
        printf("  Test 1 passed: RMSprop optimizer creation\n");
    }
    ...
    {
        boat_optimizer_t* optimizer = boat_rmsprop_optimizer_create(0.01f, 0.99f, 1e-8f);
        assert(optimizer != NULL);
        ...
        // RMSprop has adaptive learning rate, so hard to predict exact value
    ...
    {
        boat_optimizer_t* optimizer = boat_rmsprop_optimizer_create(0.1f, 0.99f, 1e-8f);
        assert(optimizer != NULL);
    ...
    printf("All RMSprop optimizer tests passed!\n");
    return 0;
}