...
static void test_matmul_broadcast_forward(void) {
    ...
    assert(boat_tensor_shape(c)[0] == 2 && boat_tensor_shape(c)[1] == 3 &&
           boat_tensor_shape(c)[2] == 5);
...
static float matmul_loss(const float* a_data, const float* b_data) {
    ...
    float sum = 0;
    float* cd = (float*)boat_tensor_data(c);
    for (size_t i = 0; i < boat_tensor_nelements(c); i++)
        sum += cd[i];
    boat_tensor_free(a);
    boat_tensor_free(b);
    boat_tensor_free(c);
...
static void test_matmul_broadcast_backward(void) {
    ...
    for (int i = 0; i < 12; i++) {
        float o = a_copy[i];
        a_copy[i] = o + eps;
        float lp = matmul_loss(a_copy, b_copy);
        a_copy[i] = o - eps;
        float lm = matmul_loss(a_copy, b_copy);
        a_copy[i] = o;
...
static void test_softmax_axis(void) {
    ...
    for (int i = 0; i < 6; i++) {
        float o = x_copy[i];
        x_copy[i] = o + eps;
        float lp = sm_loss(x_copy, 6, 0, w);
        x_copy[i] = o - eps;
        float lm = sm_loss(x_copy, 6, 0, w);
        x_copy[i] = o;
