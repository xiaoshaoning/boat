...
static void fill_block(boat_swin_block_weights_t* b, int dim) {
    ...
    b->norm1_weight = ft(v1, 1);
    b->norm1_bias = ft(v1, 1);
    b->query_weight = ft(m2, 2);
    b->query_bias = ft(v1, 1);
    b->key_weight = ft(m2, 2);
    b->key_bias = ft(v1, 1);
    b->value_weight = ft(m2, 2);
    b->value_bias = ft(v1, 1);
    b->proj_weight = ft(m2, 2);
    b->proj_bias = ft(v1, 1);
    b->norm2_weight = ft(v1, 1);
    b->norm2_bias = ft(v1, 1);
    b->mlp_fc1_weight = ft(fc1, 2);
    b->mlp_fc1_bias = ft(h1, 1);
    b->mlp_fc2_weight = ft(fc2, 2);
    b->mlp_fc2_bias = ft(v1, 1);
...
static void free_block(boat_swin_block_weights_t* b) {
    if (!b) return;
    boat_tensor_free(b->norm1_weight);
    boat_tensor_free(b->norm1_bias);
    boat_tensor_free(b->query_weight);
    boat_tensor_free(b->query_bias);
    boat_tensor_free(b->key_weight);
    boat_tensor_free(b->key_bias);
    boat_tensor_free(b->value_weight);
    boat_tensor_free(b->value_bias);
    boat_tensor_free(b->proj_weight);
    boat_tensor_free(b->proj_bias);
    boat_tensor_free(b->norm2_weight);
    boat_tensor_free(b->norm2_bias);
    boat_tensor_free(b->mlp_fc1_weight);
    boat_tensor_free(b->mlp_fc1_bias);
    boat_tensor_free(b->mlp_fc2_weight);
    boat_tensor_free(b->mlp_fc2_bias);
...
static void fill_downsample(boat_swin_downsample_weights_t* d, int dim) {
    ...
    d->norm_weight = ft(d4, 1);
    d->norm_bias = ft(d4, 1);
    d->reduction_weight = ft(red, 2);
    d->reduction_bias = ft(d2, 1);
}

static void free_downsample(boat_swin_downsample_weights_t* d) {
    if (!d) return;
    boat_tensor_free(d->norm_weight);
    boat_tensor_free(d->norm_bias);
    boat_tensor_free(d->reduction_weight);
    boat_tensor_free(d->reduction_bias);
}
...
static void test_swin_forward(void) {
    ...
    for (int s = 0; s < 4; s++) {
        ...
        if (s < 3) {
            w.stages[s].downsample =
                (boat_swin_downsample_weights_t*)malloc(sizeof(boat_swin_downsample_weights_t));
    ...
    for (size_t i = 0; i < boat_tensor_nelements(input); i++)
        id[i] = 0.05f * (float)((i * 3) % 13);
